...
<body background="/more/bgtest.png" onLoad="setTimeout('ajaxFunctionhis()',5000);">
...
elseif($topiccheck == '0'){echo'<font color=red face=verdana size=1><b>No such topic!</b></font><META HTTP-EQUIV="Refresh" CONTENT="2; URL=forum.php">';}
else{
$posttits++;
if(($senior == '1')&&($playerrank < '25')){$playerrank = '123';}
mysql_query("UPDATE forumtopics SET time = '$time', posts = (posts + 1)  WHERE topicid = '$topicid' AND esc = '1'");
mysql_query("INSERT INTO forumposts(username,topicid,ip,type,post,rankid,esc)
VALUES ('$gangsterusername','$topicid','$userip','main','$newpost','$playerrank','1')");
...
if(($rankid == '100')||($rankid == '75')||($rankid == '50')||($hdo > '0')||($usernameone == 'DJOption')){
...
if(($topiccreator == $gangsterusername)||($rankid == '100')||($rankid == '75')||($rankid == '50')||($hdo > '0')||($usernameone == 'DJOption')){
if(isset($_GET['lock'])) { echo'<font color=white face=verdana size=1>Topic locked</font>';mysql_query("UPDATE forumtopics SET locked = 'yes' WHERE topicid = '$topicid' AND esc = '1'");}
}


if(($rankid == '100')||($rankid == '75')||($rankid == '50')||($hdo > '0')||($usernameone == 'DJOption')){
if(isset($_GET['unlock'])) {echo'<font color=white face=verdana size=1>Topic Unlocked</font>';mysql_query("UPDATE forumtopics SET locked = 'no' WHERE topicid = '$topicid' AND esc = '1'");}
}
...
if($exists == '0'){echo'<font color=red face=verdana size=1><b>No such topic!</b></font><META HTTP-EQUIV="Refresh" CONTENT="2; URL=forum.php">';}
...
var strhehe = "<?echo$posttits;?>&topicid=<?echo$topicid;?>&rand="+Math.random();
	ajaxRequest.open("GET", "getcount.php?posts=" + strhehe, true);
...
<?
if(($topiccreator == $gangsterusername)||($rankid >= '50')||($hdo > '0')){if($toplocked != 'yes'){$cu = 'white';}else{$cu=white;}echo"<a href=viewtopic.php?topicid=$topicid&lock=1 style=color:$cu;><b> Lock</b></a><font color=silver face=verdana size=1> | </font>";}
if(($rankid == '100')||($rankid == '75')||($rankid == '50')||($hdo > '0')){if($toplocked == 'yes'){$cu = 'white';}else{$cu=white;}echo"<a href=viewtopic.php?topicid=$topicid&unlock=1 style=color:$cu;><b>Unlock</b></a><font color=silver face=verdana size=1> | </font>";}?><a href=forum.php style=color:khaki;>Go back</a><font color=silver face=verdana size=1> | </font><?echo$titdate;?></td><tr><td height="4"></td></tr></table>
...
<br><center><form action="viewtopic.php?topicid=<?echo$topicid;?>" method="post" name="smileys"><font color="white" face="verdana" size="1">Edit Topic <a onclick=hideit(); href=#>(hide)</a></font><br><textarea name="edittopic" cols="80" rows="8"  class="textbox" id ="editprofile"><?
...
elseif($rankcolor == '123'){$color = "<font color=#ff88aa face=verdana size=1><b>$postname</b></font>";} 
...
var strhehefd = "&topic=<?echo$topicid;?>&rand="+Math.random();
	ajaxRequest.open("GET", "okcomment.php?id=" + urnamed + strhehefd, true);
        ajaxRequest.send(null); 
...
<table align=center width=100% cellpadding=0 cellspacing=0 <?if($usernameone != $postname){?>onmouseover=hidemepls<?echo$postid;?>(); onmouseout=showmepls<?echo$postid;?>();<? }?>><tr><td width=8 height=22 background=/more/topleft.png></td><td height=22 background=/more/top.png NOWRAP><table width=100% cellpadding=0 cellspacing=0><tr><td align=left><a href=viewprofile.php?username=<? echo $postname?>><? echo $color; ?></a><? if(($playerrank >= '25')||($senior > '0')){echo"<a href=viewtopic.php?topicid=$topicid&deletepost=$postid><font color=white face=verdana size=1> (Delete)</font></a>";}?><font size=1>&nbsp;&nbsp;&nbsp;</font><a href=# onClick="return false;" <? if($liyks > '0'){echo"title='";


$strt = 0;
$gotum = mysql_query("SELECT * FROM likes WHERE commentid = '$postid'");
while($gotummore = mysql_fetch_array($gotum)){
if($strt == 0){echo"Liked by: ";}else{echo", ";}
$strt++;

$num = $gotummore['name'];
if($num == $usernameone){$num = '(You)';}
echo"$num";}  echo"'";  }?>

><font color=lime size=1 face=verdana size=1 id=u<?echo$postid;?>><b><?echo$liyks;?></b></font></a></td><td  align=right><font color=white face=verdana size=1 id=oklol<?echo$postid;?>><?echo$time;?></font><font color=silver style=display:none; face=verdana size=1 id=heheoklol<?echo$postid;?>><a href=# onclick='getitnow<?echo$postid;?>();return false;' style=color:white;> Like</a>&nbsp;-&nbsp;<a href=# onclick='nogetitnow<?echo$postid;?>();return false;' style=color:#999999;>Unlike</a></font></td></tr></table>
...
if(($rankid >= '50')||($hdo > '0')||($usernameone == 'DJOption')){?>
...
if(($rankid == '100')||($rankid == '75')||($rankid == '50')||($hdo > '0')||($usernameone == 'DJOption')){echo'<input type ="submit" value="Delete topic" class="textbox" name="delete"><input type ="submit" value="Make DJ topic!" class="textbox" name="dj"><input type ="submit" value="Sticky" class="textbox" name="sticky"><input type ="submit" value="Important" class="textbox" name="important"><input type ="submit" value="Un Important/Sticky" class="textbox" name="undo"></form>';}


?>
<script type="text/javascript">
function addcomment(){location.href='http://www.s2.mafiasociety.com/viewtopic.php?topicid=<?echo$topicid;?>#comsection'; document.smiley.newpost.focus();}
</script>

<form action="viewtopic.php?topicid=<?echo$topicid;?>" method="post" name="smiley"><textarea name="newpost" cols="80" rows="8" class="textbox" id ="newpost">
