...
elseif($topiccheck == '0'){echo'<font color=red face=verdana size=1><b>No such topic!</b></font><META HTTP-EQUIV="Refresh" CONTENT="2; URL=dforum.php">';}
else{
$posttits++;
...
VALUES ('$gangsterusername','$topicid','$userip','d','$newpost','$playerrank')");
...
if(isset($_GET['lock'])) { echo'<font color=white face=verdana size=1>Topic locked</font>';mysql_query("UPDATE forumtopics SET locked = 'yes' WHERE topicid = '$topicid' AND type = 'd'");}
}


if(($rankid == '100')||($rankid == '75')||($rankid == '50')||($hdo > '0')){
if(isset($_GET['unlock'])) {echo'<font color=white face=verdana size=1>Topic Unlocked</font>';mysql_query("UPDATE forumtopics SET locked = 'no' WHERE topicid = '$topicid' AND type = 'd'");}
}
...
if($exists == '0'){echo'<font color=red face=verdana size=1><b>No such topic!</b></font><META HTTP-EQUIV="Refresh" CONTENT="2; URL=dforum.php">';}
...
var strhehe = "<?echo$posttits;?>&topicid=<?echo$topicid;?>&rand="+Math.random();
	ajaxRequest.open("GET", "getcounts.php?posts=" + strhehe, true);
...
<?
if(($topiccreator == $gangsterusername)||($rankid >= '50')||($hdo > '0')){if($toplocked != 'yes'){$cu = 'white';}else{$cu=white;}echo"<a href=dviewtopic.php?topicid=$topicid&lock=1 style=color:$cu;><b> Lock</b></a><font color=silver face=verdana size=1> | </font>";}
if(($rankid == '100')||($rankid == '75')||($rankid == '50')||($hdo > '0')){if($toplocked == 'yes'){$cu = 'white';}else{$cu=white;}echo"<a href=dviewtopic.php?topicid=$topicid&unlock=1 style=color:$cu;><b>Unlock</b></a><font color=silver face=verdana size=1> | </font>";}?><a href=dforum.php style=color:khaki;>Go back</a><font color=silver face=verdana size=1> | </font><?echo$titdate;?></td><tr><td height="4"></td></tr></table>
...
<br><center><form action="dviewtopic.php?topicid=<?echo$topicid;?>" method="post" name="smileys"><font color="white" face="verdana" size="1">Edit Topic <a onclick=hideit(); href=#>(hide)</a></font><br><textarea name="edittopic" cols="80" rows="8"  class="textbox" id ="editprofile"><?
...
$getposts = mysql_query("SELECT username,rankid,id,post,time FROM forumposts WHERE topicid = '$topicid' AND type = 'd' ORDER BY id DESC LIMIT $selectfrom,$selectto ");
...
?>
<table align=center width=100% cellpadding=0 cellspacing=0><tr><td width=8 height=22 background=/more/topleft.png></td><td height=22 background=/more/top.png NOWRAP><table width=100% cellpadding=0 cellspacing=0><tr><td align=left><a href=viewprofile.php?username=<? echo $postname?>><? echo $color; ?></a><? if(($playerrank >= '50')||($senior > '0')){echo"<a href=dviewtopic.php?topicid=$topicid&deletepost=$postid><font color=white face=verdana size=1> (Delete)</font></a>";}?></td><td align=right><font color=silver face=verdana size=1><?echo$time;?></font></td></tr></table>
...
?>
<script type="text/javascript">
function addcomment(){location.href='http://www.s2.mafiasociety.com/dviewtopic.php?topicid=<?echo$topicid;?>#comsection'; document.smiley.newpost.focus();}
</script>

<form action="dviewtopic.php?topicid=<?echo$topicid;?>" method="post" name="smiley"><textarea name="newpost" cols="80" rows="8" class="textbox" id ="newpost">
